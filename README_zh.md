...
2. `drp_dip`
   按照原始 DRP-DIP 论文的形式设置：冻结随机初始化主干参数，只优化输入 seed、BatchNorm 仿射参数，并结合 TV 正则。

3. `as_dip`
   在 DRP-DIP 基线上加入分阶段优化：先预热 latent seed，再训练 adapter 和归一化层，最后逐步解冻 backbone，并在后期降低 latent 学习率、结合残差信号泄露风险监控。

可以简单理解为：

- `standard_dip` = 传统 DIP 基线
- `drp_dip` = 原始论文形式的 DRP-DIP
- `as_dip` = 带分阶段优化和渐进解冻的增强版 DRP 路线
